...
const { isValidRequestBody, isValid, isValidObjectId, isValidImg } = require("../validator/validation");
...
const createProducts = async (req, res) => {
    try {
        ...
        if (currencyFormat != "₹") {
            return res
                .status(400)
                .send({ status: false, message: "Currency Symbol should be only in '₹'" });
        }
        ...
        if (isFreeShipping != null) {
            if (
                ...
                return res
                    .status(400)
                    .send({ status: false, message: "Please Provide only Boolean Value" });
            }
...
const updateProduct = async function (req, res) {
    try {
        ...
        if (availableSizes != null) {
            ...
            //console.log(arr) returns whole size array
            for (let i = 0; i < arr.length; i++) {
                if (arr.includes(sizeArr)) {
                    return res.status(404).send({
                        status: false,
                        message: `This ${sizeArr} size is Already Present `,
                    });
                }
            }
        }
        ...
        if (files) {
            ...
            if (files && files.length > 0) {
                ...
                //console.log(newObj["productImage"])
             } 
        }
